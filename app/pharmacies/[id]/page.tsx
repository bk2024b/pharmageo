...
export default async function PharmacyPage({ params }: Props) {
  ...
  // ── Fetch en parallèle ────────────────────────────────────────────────
  const [pharmacy, inventory] = await Promise.all([
    getPharmacyById(id),
    getMedicationsByPharmacy(id),
  ])
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `${pharmacy.name} — PharmaGéo`,
    description: `${pharmacy.address}${pharmacy.is_on_guard ? ' · De garde' : ''}`,
  }
}